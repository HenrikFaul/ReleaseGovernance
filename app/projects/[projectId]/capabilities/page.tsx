...
function CapabilityCard({
  ...
  capability: NonNullable<ReturnType<typeof useProjectRecord>["project"]>["capabilities"][number];
  projectName: string;
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-sm text-slate-500">
            ...
            )) : <span>{projectName} feature without Jira</span>}
          </div>
        </div>
  ...
  return (
    <AppShell projectId={project.id}>
      <div className="space-y-6">
        <SectionHeader eyebrow="Capability registry" title={`${project.name} features`} description="Track functions independently from commits and split them into released, unreleased and deployed-without-Jira categories." />

        <section className="space-y-4">
          <div className="card p-5">
            <h3 className="text-lg font-semibold text-slate-900">Released features</h3>
            <p className="mt-2 text-sm text-slate-600">Capabilities already represented by at least one deployed release record.</p>
          </div>
          {released.map((capability) => <CapabilityCard key={capability.id} capability={capability} projectName={project.name} tone="success" />)}
        </section>

        <section className="space-y-4">
          <div className="card p-5">
            <h3 className="text-lg font-semibold text-slate-900">Unreleased features</h3>
            <p className="mt-2 text-sm text-slate-600">Capabilities tracked in backlog or future release trains but not yet represented by a deployed release.</p>
          </div>
          {unreleased.length ? unreleased.map((capability) => <CapabilityCard key={capability.id} capability={capability} projectName={project.name} tone="info" />) : (
            <div className="card p-6 text-sm text-slate-600">No unreleased capability rows.</div>
          )}
        </section>
            ...
            <div key={candidate.id} className="card p-6">
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-3 text-sm text-slate-600">
                <div className="rounded-2xl bg-slate-50 p-4">
                  <div className="text-slate-500">Suggested parent</div>
                  <div className="mt-1 font-medium text-slate-900">{candidate.parent}</div>
                </div>
                <div className="rounded-2xl bg-slate-50 p-4">
                  <div className="text-slate-500">Issue type</div>
                  <div className="mt-1 font-medium text-slate-900">{candidate.issueType}</div>
                </div>
                <div className="rounded-2xl bg-slate-50 p-4">
                  <div className="text-slate-500">Recommended release</div>
                  <div className="mt-1 font-medium text-slate-900">{candidate.recommendedRelease ?? "—"}</div>
                </div>
              </div>
            </div>
          )) : (
