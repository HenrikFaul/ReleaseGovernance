...
export default function ReleaseDetailPage({ params }: { params: { projectId: string; releaseId: string } }) {
  ...
  const capabilityLookup = new Map(project.capabilities.map((item) => [item.id, item]));

  return (
    <AppShell projectId={project.id}>
      <div className="space-y-6">
        <SectionHeader eyebrow="Release detail" title={release.version} description="Inspect delivered functionality, connected Jira work and cross-platform impact." actions={<StatusBadge tone={release.releaseState === "unreleased" ? "info" : impact.complianceStatus === "ready" ? "success" : impact.complianceStatus === "needs-follow-up" ? "warning" : "danger"}>{release.releaseState === "unreleased" ? "unreleased" : impact.complianceStatus}</StatusBadge>} />
        <div className="grid gap-6 xl:grid-cols-[1fr,1fr]">
          <section className="card p-6">
            ...
            <div className="mt-4 space-y-4 text-sm text-slate-700">
              ...
              <div>{release.releaseState === "unreleased" ? "Planning state" : "Shipped at"}: {release.shippedAt}</div>
          ...
          <section className="card p-6">
            ...
            <ul className="mt-4 space-y-2 text-sm text-slate-700">
              {release.deliveredCapabilities.map((capabilityId) => {
                const capability = capabilityLookup.get(capabilityId);
                return (
                  <li key={capabilityId} className="rounded-xl bg-slate-50 px-4 py-3">
                    <div className="font-medium text-slate-900">{capability?.name ?? capabilityId}</div>
                    {capability?.description ? <div className="mt-1 text-slate-500">{capability.description}</div> : null}
                    {capability?.jiraKeys?.length ? <div className="mt-1 text-xs text-slate-500">Jira: {capability.jiraKeys.join(", ")}</div> : null}
                  </li>
                );
              })}
            </ul>
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr,1fr]">
          ...
          <section className="card p-6">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              ...
              <div>Compliance status: {release.releaseState === "unreleased" ? "unreleased" : impact.complianceStatus}</div>
            </div>
          </section>
        </div>
      </div>
