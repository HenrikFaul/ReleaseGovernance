...
import { globalNavItems, projectNavItems } from "../../lib/navigation/nav-model";
        ...
        <aside className="hidden w-[280px] shrink-0 overflow-hidden rounded-[32px] bg-gradient-to-b from-slate-950 via-slate-900 to-teal-950 p-4 text-white lg:block">
          <nav className="mt-6 space-y-2">
            {globalNavItems.map((item) => (
              <Link key={item.key} href={item.href} className={itemClass(pathname === item.href)}>{item.label}</Link>
            ))}
          </nav>
          {projectId ? (
            <nav className="mt-6 space-y-2">
              {projectNavItems.map((item) => {
                ...
                return <Link key={item.key} href={href} className={itemClass(pathname === href)}>{item.label}</Link>;
              })}
            </nav>
          ) : null}
        </aside>
