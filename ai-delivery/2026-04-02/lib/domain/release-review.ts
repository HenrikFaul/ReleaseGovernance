...
export function evaluateReleaseForApproval(release: Release, surfaces: ReleaseSurface[]): ReleaseApprovalEvaluation {
  const checks = [hasSource(release), hasNotes(release), confidenceIsKnown(release), atLeastOneSurface(surfaces)];
  return { canApprove: checks.every((item) => item.passed), checks };
}
